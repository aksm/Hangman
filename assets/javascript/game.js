...
var game = {
	...
	current_word: document.getElementById("current-word"),
	win_count: document.getElementById("wins"),
	guess_count: document.getElementById("guesses"),
	hangman: document.getElementById("hangman"),

	// Game functions
	// Fetch new word and initialize variables and display
	newWord: function() {
        var requestStr = "https://crossorigin.me/http://randomword.setgetgo.com/get.php";
        ...
        $.ajax({
			...
			// this.word = this.dictionary[[Math.floor(Math.random() * this.dictionary.length)]].toUpperCase();
			...
			for(i = 0; i < this.word.length; i++) {
				this.current_word.innerHTML = this.current_word.innerHTML + "_"
				this.word_guess.push("_");
				this.blank_counter++;
			}
	...
	guess: function(event) {
		...
		if (event.keyCode > 64 && event.keyCode < 91 && this.win_state == false){
			this.letter = String.fromCharCode(event.keyCode);
			this.guessed = true;
			// Check if letter is already guessed and if not add to array
			if(jQuery.inArray(this.letter,this.letters_a) === -1) {
				this.letters_a.push(this.letter);
				this.guessed = false;
			}
			// Update word blanks and guessed letters display and reset guessed letter counter
			this.letters.innerHTML = this.letters_a;
			this.current_word.innerHTML = "";
			this.letter_counter = 0;
			// Loop for guess letter check against word
			for(i = 0; i < this.word.length; i++) {
			...
			// Logic for game win
			if(this.blank_counter == 0 && this.win_state == false) {
				this.win_state = true;
				this.wins++;
				this.win_count.innerHTML = this.wins;
				this.playAudio();
